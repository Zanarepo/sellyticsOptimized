...
const [showInstructions, setShowInstructions] = useState(false);
  ...
  const removeDeviceId = async (deviceId) => {
    ...
    if (!window.confirm(`Remove device ID ${deviceId} from ${showDetail.name || 'this product'}?`)) return;
    try {
      ...
      const updatedDeviceList = deviceList.filter((id) => id !== deviceId);
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          purchase_qty: updatedDeviceList.length > 0 ? updatedDeviceList.length : (showDetail.purchase_qty || 0),
        })
        .eq('id', showDetail.id);
      if (error) throw error;
